            ...
            <div class="space-y-4">
                ...
                @else
                    <div>
                        ...
                        @error('selected_template_id') <p class="text-xs text-rose-600">{{ $message }}</p> @enderror
                        <div class="mt-2 flex flex-wrap gap-2">
                            <x-ui.button size="sm" type="button" wire:click="generate">Gerar rascunho</x-ui.button>
                            <x-ui.button size="sm" type="button" variant="secondary" wire:click="saveDraft" @if (! $active_notice_id) disabled @endif>Guardar rascunho</x-ui.button>
                            <x-ui.button size="sm" type="button" variant="secondary" wire:click="markSent" @if (! $active_notice_id) disabled @endif>Marcar enviado</x-ui.button>
                        </div>
                    </div>
