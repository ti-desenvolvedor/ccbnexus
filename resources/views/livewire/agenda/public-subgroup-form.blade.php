<x-ui.card title="{{ $this->publicSubgroup ? __('Editar subgrupo') : __('Novo subgrupo') }} — {{ $group->name }}">
    <form wire:submit="save" class="grid max-w-md gap-4">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('agenda.public-subgroups.index', $group) }}" wire:navigate class="inline-flex items-center rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold dark:border-slate-800">{{ __('Cancelar') }}</a>
        </div>
    </form>
</x-ui.card>
