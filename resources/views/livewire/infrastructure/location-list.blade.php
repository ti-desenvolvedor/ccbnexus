            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($locations as $row)
                        <tr wire:key="loc-{{ $row->id }}">
                            <td class="px-4 py-3 font-medium">{{ $row->name }}</td>
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-300">{{ $row->formattedAddressSummary() }}</td>
                            <td class="px-4 py-3 text-right">
                                @can('update', $row)
                                    <a href="{{ route('infrastructure.locations.edit', $row) }}" wire:navigate class="text-primary-600 hover:underline">Editar</a>
                                @endcan
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="3" class="px-4 py-8 text-center text-slate-500">Nenhum registo.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
